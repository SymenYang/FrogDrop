...
from BackEnd import broadcaster, data, listener, protocol, sender, receiver
import sys, time, os
...
class FrogDrop(QtWidgets.QMainWindow, Ui_FrogDrop):
    ...
    def show_Message_Box(self):
        ok = (QMessageBox.Yes == QMessageBox.question(self,'New Request','Do you want to receive %s from %s?' % (fd_data.fileName, fd_data.reqName),QMessageBox.Yes | QMessageBox.No))
        receiver.finishRecon(ok, self.restart_listen)
...
class SendDialog(QtWidgets.QDialog, Ui_SendDialog):
    ...
    def send(self):
        ...
        # while not sender.stopFlag:
        # 	self.ProgBar.setValue(fd_data.sentSize * 1.0 / fd_data.fileSize * 100)

        sender.startSend(self.ip, self.file, sender.sendFile)
        # self.work_thread = SendThread(self.ip, self.file)
        # self.work_thread.trigger.connect(self.send_complete)
